    ...
    <Form onSubmit={handleSubmit(onSubmit)}>
      ...
      <FormRow label="Password (min 8 characters)" error={errors?.password?.message}>
        <Input disabled={isLoading} type="password" id="password"  {...register("password",{
          ...
          minLength : {
            value : 8,
            message : 'Password must have greater than 7'
          }
        })}/>
      </FormRow>

      <FormRow label="Repeat password" error={errors?.passwordConfrim?.message}>
        <Input disabled={isLoading} type="password" id="passwordConfirm"  {...register("passwordConfrim",{
          required : "This field is required",
